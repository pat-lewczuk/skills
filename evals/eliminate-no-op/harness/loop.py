...
One iteration:
    ...
    -> overfit guard             ->  analyze.py smoke test
...
never touched unless you pass --promote.
    ...
    python3 harness/loop.py --iterations 1 --promote                  # copy a winner into skills/
"""
...
def scores(run_dir: Path) -> dict:
    return json.loads((run_dir / "scores.json").read_text(encoding="utf-8"))["summary"]


def smoke_test(skill_dir: Path) -> tuple:
    """analyze.py must still run, stdlib-only, against this repo's own AGENTS.md."""
    proc = subprocess.run(
        [PY, str(skill_dir / "scripts" / "analyze.py"), "AGENTS.md", "--repo", ".",
         "--json", "/dev/null", "--quiet"],
...
def render_scoreboard(data: list | None = None) -> None:
    """Rebuild scoreboard.md from scoreboard.json. Callable on its own after hand-editing
    a `reason` — the history is append-only, so corrections get annotated, not deleted."""
    if data is None:
    ...
    for i, e in enumerate(data):
        lines.append(
            f"| {i} | {e['when']} | {e['label']} | {_f(e.get('train'))} | "
            f"{_f(e.get('holdout'))} | {_f(e.get('adjusted'))} | {e.get('skill_md_chars', '—')} | "
...
def main() -> int:
    ...
    ap.add_argument("--promote", action="store_true",
                    help="copy an accepted candidate over the installed skill")
    ap.add_argument("--evaluate", default=None,
                    help="skill directory to evaluate as the first candidate instead of "
                         "generating one — for a hand-written revision, or to re-judge a "
                         "candidate the optimiser already produced")
    args = ap.parse_args()
    ...
    best_skill = Path(args.skill_dir).resolve()
    ...
    base = scores(best_run)
    best_train, best_holdout = base["train_score"], base["holdout_score"]
    if best_train is None or best_holdout is None:
        print("error: the baseline must cover both splits — run the full suite, or point "
              "--baseline-run at a run that did", file=sys.stderr)
        return 1
    append_scoreboard({
        "when": time.strftime("%Y-%m-%d %H:%M"), "label": "baseline",
        "train": best_train, "holdout": best_holdout, "adjusted": base["adjusted_score"],
        "skill_md_chars": len((best_run / "skill-snapshot" / "SKILL.md").read_text(encoding="utf-8")),
        "guard_ok": True, "verdict": "baseline", "run_dir": str(best_run),
        "reason": "metric means — " + " · ".join(
            f"{k.replace('_', ' ')} {v:.2f}" for k, v in base["metric_means"].items()
            if v is not None),
    })
    ...
    for i in range(1, iterations + 1):
        ...
        if i == 1 and args.evaluate:
            ...
            subprocess.run(
                [PY, str(ROOT / "harness" / "improve.py"), "--run", str(best_run),
                 "--candidate", str(cand_root)]
                + (["--model", args.model] if args.model else []),
                cwd=str(ROOT), check=False)
            cand_skill = cand_root / "eliminate-no-op"
        if not (cand_skill / "SKILL.md").exists():
            print("optimiser produced no candidate; stopping")
            break

        verdict = guard.check(best_run / "skill-snapshot", cand_skill, ROOT / "cases",
                              [c["id"] for c in C.load_cases(ROOT)])
        chars = len((cand_skill / "SKILL.md").read_text(encoding="utf-8"))
        entry = {"when": time.strftime("%Y-%m-%d %H:%M"), "label": label,
                 "skill_md_chars": chars, "guard_ok": verdict["ok"],
                 "candidate": str(cand_skill), "train": None, "holdout": None,
                 "adjusted": None}
        ...
        ok, err = smoke_test(cand_skill)
        if not ok:
            entry.update({"verdict": "rejected", "reason": f"analyze.py smoke test failed: {err[:200]}"})
            append_scoreboard(entry)
            print(f"rejected — analyze.py no longer runs:\n{err}")
            continue

        print(f"\n=== iteration {i}: re-measure train ===")
        cand_train_run = run_suite(cand_skill, f"{label}-train", "train", args.model)
        cand_train = scores(cand_train_run)["train_score"]
        gain = cand_train - best_train
        entry["train"] = cand_train
        entry["run_dir"] = str(cand_train_run)
        ...
        cand_hold_run = run_suite(cand_skill, f"{label}-holdout", "holdout", args.model)
        cand_hold = scores(cand_hold_run)["holdout_score"]
        regress = best_holdout - cand_hold
        entry["holdout"] = cand_hold
        entry["holdout_run_dir"] = str(cand_hold_run)
        print(f"holdout {cand_hold} vs {best_holdout} (change {-regress:+.4f})")

        if regress > max_regress:
            entry.update({"verdict": "rejected",
                          "reason": f"train +{gain:.4f} but holdout -{regress:.4f}: "
                                    f"the revision fits the training files, not the method"})
            append_scoreboard(entry)
            continue
        ...
        append_scoreboard(entry)
        print("accepted")
        best_skill, best_train, best_holdout = cand_skill, cand_train, cand_hold
        # the next digest reads from the run that measured this candidate
        best_run = cand_train_run
        shutil.copytree(cand_skill, best_run / "skill-snapshot", dirs_exist_ok=True,
                        ignore=C.IGNORE)

    print(f"\nbest: {best_skill}\n  train {best_train} · holdout {best_holdout}")
    if args.promote and best_skill.resolve() != Path(args.skill_dir).resolve():
        target = Path(args.skill_dir).resolve()
        shutil.rmtree(target)
        shutil.copytree(best_skill, target, ignore=C.IGNORE)
        print(f"promoted into {target} — review with `git diff` before committing")
    elif args.promote:
